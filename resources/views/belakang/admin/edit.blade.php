                ...
                <div class="form-group">
                    <div class="form-row">
                        <div class="col-md-9" style="text-align: left;"> 
                            ...
                            <a href="{{route('admin.index')}}" class="active"><i class="fa fa-fw fa-table"></i>Data User </a>
                            <a><i class="fa fa-edit"></i> Ubah Data User :{!!' <strong>'.$admin->name.'</strong>'!!}</a> 
                        </div> 
            ...
            <div class="card-body">
                <form method="post" action="{{ route('admin.update', $admin->id) }}" data-parsley-validate class="form-horizontal form-label-left" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5 form-group{{ $errors->has('posisi') ? ' has-error' : '' }}"  >
                            ...
                            <div>
                                <select class="form-control" name="posisi" id="posisi">
                                    @foreach($posisi as $v)
                                    <option value="{{ $v }}" {{$admin->posisi == $v ? 'selected="selected"' : ''}}> {{ $v}} </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-10 form-group{{ $errors->has('aturan') ? ' has-error' : '' }}" hidden="true">
                            <label class="control-label" for="aturan">aturan <span class="required">*</span>
                            </label>
                            <div>
                                <input type="text" value="{{$admin->aturan}}" id="aturan" name="aturan" class="form-control">
                                @if ($errors->has('aturan'))
                                <span class="help-block">{{ $errors->first('aturan') }}</span>
                                @endif
                            </div>
                        </div>
